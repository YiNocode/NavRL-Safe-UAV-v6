...
class StabilityOnPolicyRunner(OnPolicyRunner):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.best_metric_window = int(self.cfg.get("best_metric_window", 25))
        self.best_metric_min_delta = float(self.cfg.get("best_metric_min_delta", 0.002))
        self.early_stopping_patience = int(self.cfg.get("early_stopping_patience", 500))
        self.early_stopping_warmup = int(self.cfg.get("early_stopping_warmup", 50))
        self.early_stopping_max_drop = float(self.cfg.get("early_stopping_max_drop", 0.08))
        self.early_stopping_degradation_patience = int(
            self.cfg.get("early_stopping_degradation_patience", 50)
        )
        if self.best_metric_window < 1:
            raise ValueError("best_metric_window must be positive")
        if self.early_stopping_patience < 1 or self.early_stopping_degradation_patience < 1:
            raise ValueError("early-stopping patience values must be positive")
        ...
        self._metric_history: deque[float] = deque(maxlen=self.best_metric_window)
        self._best_metric = -math.inf
        self._best_iteration = -1
        self._last_improvement_iteration = -1
        self._first_metric_iteration: int | None = None
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35) -> None:
        ...
        metric = self._extract_episode_metric(locs.get("ep_infos", []))
        if metric is None:
            return
        iteration = int(locs["it"])
        if self._first_metric_iteration is None:
            self._first_metric_iteration = iteration
        self._metric_history.append(metric)
        if len(self._metric_history) < self.best_metric_window:
            return
        ...
        if rolling_metric > self._best_metric + self.best_metric_min_delta:
            ...
            self.save(
                ...
                infos={"best_metric": self._best_metric, "best_iteration": iteration},
            )
        ...
        updates_seen = iteration - self._first_metric_iteration + 1
        if updates_seen < self.early_stopping_warmup:
            return
        if self._last_improvement_iteration < 0:
            self._last_improvement_iteration = iteration

        reason: str | None = None
        if self._degradation_count >= self.early_stopping_degradation_patience:
            reason = (
                ...
                f"{self._degradation_count} updates"
            )
        elif iteration - self._last_improvement_iteration >= self.early_stopping_patience:
            reason = (
                f"no rolling-metric improvement greater than {self.best_metric_min_delta:.4f} "
                f"for {self.early_stopping_patience} iterations"
            )
        if reason is None:
            return
    ...
    def _extract_episode_metric(self, ep_infos: list[dict]) -> float | None:
